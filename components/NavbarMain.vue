...
<section id="navbar">
    <v-container fluid class="ma-0 pa-0">
      <v-app-bar fixed elevate-on-scroll color="white" height="69px" class="px-16">
 
          <nuxt-link to="/">
              <v-img src="Kanzler.png" width="130px" class="ml-10 pa-0"></v-img>
          </nuxt-link>

                <v-spacer></v-spacer>

          <v-toolbars>
              <v-toolbar-items class="text-decoration-none hidden-xs-only d-sm-flex">          
                  <v-list-item class="pa-0 align-center">

                    <nuxt-link class="d-flex justify-center" to="/"> 
                      <v-items class="d-flex justify-center" @click="scrollToHome()" > Home </v-items>
                    </nuxt-link>

                    <a v-for="(items,i) in linkbar" :key="i" class="d-flex justify-center align-center">
                      <v-items class="d-flex justify-center" @click="scrollTo(items.link)" > {{ items.title }} </v-items>
                    </a>       

                  </v-list-item>
              </v-toolbar-items>
           
              <v-btn class="hidden-sm-and-up pl-4" icon @click="Clicklist = !Clicklist">
          ...
          <v-toolbars class="mx-10 ml-13">
            <nuxt-link to="AdminPage">
               <v-btn outlined small color="#342719" class="py-5 px-4" > Admin </v-btn>
            </nuxt-link>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      linkbar: [
        ...
        { title: "Facilities", icon: "", link:"room" },
  ...
  methods: {
    ...
    scrollToHome() {document.getElementById("home").scrollIntoView({ behavior: "smooth" });
    this.Clicklist = false;
    },
